...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    # Train for one epoch
    model.train_model(training_dataloader, validation_dataloader, test_dataloader,
                      loss_function, optimizer, epochs=1, save_path=SAVE_PATH, save_every=SAVE_EVERY)
    
    # Calculate accuracies
    train_acc = calculate_accuracy(model, training_dataloader, DEVICE)
    val_acc = calculate_accuracy(model, validation_dataloader, DEVICE)
